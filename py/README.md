...
The Python SDK for the Civicapi API — an entity-oriented client following Pythonic conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.Election()` — each
...
```python
results = client.Result().list({"election_id": "example"})
```
...
```
py/
├── civicapi_sdk.py         -- Main SDK module
