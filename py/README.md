...
The Python SDK for the Civicapi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Election(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Civicapi(None).load(
    {"id": "test01"}, None
)
