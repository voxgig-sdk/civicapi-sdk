...
class TestPollingDirect:

    def test_should_direct_list_polling(self):
        setup = _polling_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-polling", "live" if setup["live"] else "unit")
        if _skip:
...
def _polling_direct_setup(mockres):
