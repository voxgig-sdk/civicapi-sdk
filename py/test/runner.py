...
class CivicapiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CivicapiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CivicapiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CivicapiTestRunner.getenv("CIVICAPI_TEST_LIVE")
        override = CivicapiTestRunner.getenv("CIVICAPI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CivicapiTestRunner.getenv(key)
        ...
        explain = CivicapiTestRunner.getenv("CIVICAPI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CIVICAPI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CivicapiTestRunner._test_control is not None:
            return CivicapiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CivicapiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CivicapiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CivicapiTestRunner.load_test_control()
...
def load_env_local():
    CivicapiTestRunner.load_env_local()


def env_override(m):
    return CivicapiTestRunner.env_override(m)


def entity_data(v):
    return CivicapiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CivicapiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CivicapiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CivicapiTestRunner.load_test_control()


def live_client_options():
    return CivicapiTestRunner.live_client_options()


def live_delay_ms():
    return CivicapiTestRunner.live_delay_ms()
