...
from civicapi_sdk.civicapi_types import (
    Election,
    ElectionListMatch,
)
...
class ElectionEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ElectionEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Election]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Election().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
