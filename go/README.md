...
The Golang SDK for the Civicapi API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/civicapi-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/civicapi-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/civicapi-sdk/go=../civicapi-sdk/go
```
...
func main() {
    ...
    // List election records — the value is the array of records itself.
    elections, err := client.Election(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range elections.([]any) {
        fmt.Println(item)
    }
}
```
...
election, err := client.Election(nil).List(
    nil, nil,
)
if err != nil {
    panic(err)
}
fmt.Println(election) // the returned mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *CivicapiSDK
```
...
| `Election` | `(data map[string]any) CivicapiEntity` | Create an Election entity instance. |
| `Polling` | `(data map[string]any) CivicapiEntity` | Create a Polling entity instance. |
| `Result` | `(data map[string]any) CivicapiEntity` | Create a Result entity instance. |

### Entity interface (CivicapiEntity)

All entities implement the `CivicapiEntity` interface.
...
slice):
    ...
    election, err := client.Election(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // election is the returned record
...
Create an instance: `polling := client.Polling(nil)`
...
```go
pollings, err := client.Polling(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(pollings) // the array of records
```
...
```
github.com/voxgig-sdk/civicapi-sdk/go/
├── civicapi.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/civicapi-sdk/go`) re-exports everything needed
...
election.List(nil, nil)

// election.Data() now returns the election data from the last list
// election.Match() returns the last match criteria
```
