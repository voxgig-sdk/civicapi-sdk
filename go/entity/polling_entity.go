...
func NewPollingEntity(client *core.CivicapiSDK, entopts map[string]any) *PollingEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *PollingEntity) GetName() string { return e.name }
...
func (e *PollingEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *PollingEntity) Deleted() bool {
	return e.deleted
}


func (e *PollingEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewPollingEntity(e.client, opts)
}

func (e *PollingEntity) Data(args ...any) any {
...
func (e *PollingEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Polling; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *PollingEntity) DataTyped(data ...Polling) Polling {
	if len(data) > 0 {
		return typedFrom[Polling](e.Data(asMap(data[0])))
	}
	return typedFrom[Polling](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Polling (all fields
// optional at the wire level).
func (e *PollingEntity) MatchTyped(match ...Polling) Polling {
	if len(match) > 0 {
		return typedFrom[Polling](e.Match(asMap(match[0])))
	}
	return typedFrom[Polling](e.Match())
}
...
func (e *PollingEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *PollingEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}



func (e *PollingEntity) List(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// PollingListMatch and returns []Polling. It delegates to the untyped
// List (identical runtime) and converts at the typed boundary.
func (e *PollingEntity) ListTyped(reqmatch PollingListMatch, ctrl map[string]any) ([]Polling, error) {
	res, err := e.List(asMap(reqmatch), ctrl)
	if err != nil {
		return nil, err
	}
	return typedSliceFrom[Polling](res), nil
}



func (e *PollingEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *PollingEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *PollingEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *PollingEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
