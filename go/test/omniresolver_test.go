...
import (
	...
	omni "github.com/voxgig-sdk/civicapi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CivicapiError); is {
				out := map[string]any{
					"name":    "CivicapiError",
...
func omniCtx(arg any, client *sdk.CivicapiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CivicapiSDK); is {
			client = live
