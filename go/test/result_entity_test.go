...
func TestResultEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		resultRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.result", setup.data)))
...
func resultBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"result01", "result02", "result03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CIVICAPI_TEST_RESULT_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CIVICAPI_TEST_RESULT_ENTID"])
