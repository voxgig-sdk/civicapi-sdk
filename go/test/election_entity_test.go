...
func TestElectionEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Election(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Election(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		electionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.election")))
...
func electionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CIVICAPI_TEST_ELECTION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CIVICAPI_TEST_ELECTION_ENTID"])
