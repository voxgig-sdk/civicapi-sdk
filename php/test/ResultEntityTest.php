...
class ResultEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set CIVICAPI_TEST_RESULT_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        [$result_ref01_list_result, $err] = $result_ref01_ent->list($result_ref01_match, null);
        $this->assertNull($err);
...
function result_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["result01", "result02", "result03"] as $k) {
    ...
    $entid_env_raw = getenv("CIVICAPI_TEST_RESULT_ENTID");
