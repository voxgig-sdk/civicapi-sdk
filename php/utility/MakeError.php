...
class CivicapiMakeError
{
    public static function call(?CivicapiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CivicapiContext([], null);
        }
        $op = $ctx->op ?? new CivicapiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CivicapiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CivicapiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CivicapiError('', $msg, $ctx);
