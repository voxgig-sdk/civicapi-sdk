...
The PHP SDK for the Civicapi API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Election()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
print_r(array_map(fn($item) => $item->data_get(), $election));
```
...
```php
$client = CivicapiSDK::test($testopts, $sdkopts);
```
...
```
php/
├── civicapi_sdk.php          -- Main SDK class
...
The main class (`civicapi_sdk.php`) exports the SDK class
