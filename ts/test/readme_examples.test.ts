...
//     (\`new CivicapiSDK(...)\` / \`CivicapiSDK.test(...)\` become
//     \`CivicapiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"election":{"test01":{"id":"test01"}},"polling":{"test01":{"id":"test01"}},"result":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CivicapiSDK(...)\` and any
// \`CivicapiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CivicapiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CivicapiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
