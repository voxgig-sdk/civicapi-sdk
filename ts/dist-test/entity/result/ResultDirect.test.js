...
(0, node_test_1.describe)('ResultDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-result', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-result', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            query.election_id = "2024-presidential";
        }
...
function liveScenariosActive() { return false && process.env.CIVICAPI_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.CivicapiSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['CIVICAPI_TEST_RESULT_ENTID'];
    ...
    const client = new __1.CivicapiSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=ResultDirect.test.js.map