...
[MCP](https://modelcontextprotocol.io) server exposing the Civicapi SDK as
two agent tools — `civicapi_list` and `civicapi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/civicapi-mcp)
make build

# 2. Provide credentials via the environment
export CIVICAPI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user civicapi \
  -- /absolute/path/to/civicapi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./civicapi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// civicapi_list: first page of records
{ "entity": "election" }
{ "entity": "election", "query": { } }

// civicapi_load: one record by id
{ "entity": "election", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/civicapi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user civicapi \
     -- "$PWD"/dist/*/civicapi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `civicapi_list` and `civicapi_load` tools now appear
   in new sessions. Ask the agent to *"list election using civicapi"*
   and it calls `civicapi_list` with `{"entity":"election"}`.
...
```sh
export CIVICAPI_APIKEY=sk_live_xxx            # API key
export CIVICAPI_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `civicapi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `civicapi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CIVICAPI_APIKEY` | API key sent with every request. |
| `CIVICAPI_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

election | polling | result

### Smoke test via HTTP (raw JSON-RPC)

```sh
./civicapi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"civicapi_load","arguments":{"entity":"election","query":{"id":1}}}}'
```
