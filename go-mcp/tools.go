...
type Args struct {
	Entity string         `json:"entity" jsonschema:"election | polling | result"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.CivicapiSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "civicapi_list",
		Description: "List records from Civicapi. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "civicapi_load",
		Description: "Load a single record from Civicapi. " +
...
func runOp(client *sdk.CivicapiSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.CivicapiSDK, name string) (sdk.CivicapiEntity, error) {
	switch strings.ToLower(name) {
	case "election":
		return client.Election(nil), nil
	case "polling":
		return client.Polling(nil), nil
	case "result":
