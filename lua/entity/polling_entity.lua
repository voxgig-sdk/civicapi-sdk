...
local PollingEntity = {}
PollingEntity.__index = PollingEntity


function PollingEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PollingEntity)
  self._name = "polling"
  self._client = client
...
function PollingEntity:get_name()
  return self._name
end


function PollingEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PollingEntity.new(self._client, opts)
end
...
function PollingEntity:mark_deleted()
  self._deleted = true
end


function PollingEntity:deleted()
  return true == self._deleted
end


function PollingEntity:data_set(args)
  if args ~= nil then
...
function PollingEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PollingEntity:match_set(args)
  if args ~= nil then
...
function PollingEntity:match_get()
...
function PollingEntity:stream(action, args, callopts)
...
function PollingEntity:_run_op(ctx, post_done)
  local utility = self._utility
