...
local ElectionEntity = {}
ElectionEntity.__index = ElectionEntity


function ElectionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ElectionEntity)
  self._name = "election"
  self._client = client
...
function ElectionEntity:get_name()
  return self._name
end


function ElectionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ElectionEntity.new(self._client, opts)
end
...
function ElectionEntity:mark_deleted()
  self._deleted = true
end


function ElectionEntity:deleted()
  return true == self._deleted
end


function ElectionEntity:data_set(args)
  if args ~= nil then
...
function ElectionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ElectionEntity:match_set(args)
  if args ~= nil then
...
function ElectionEntity:match_get()
...
function ElectionEntity:stream(action, args, callopts)
...
function ElectionEntity:_run_op(ctx, post_done)
  local utility = self._utility
