-- Civicapi SDK Result entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local ResultEntity = {}
ResultEntity.__index = ResultEntity


function ResultEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ResultEntity)
  self._name = "result"
  self._client = client
...
function ResultEntity:get_name()
  return self._name
end


function ResultEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ResultEntity.new(self._client, opts)
end
...
function ResultEntity:mark_deleted()
  self._deleted = true
end


function ResultEntity:deleted()
  return true == self._deleted
end


function ResultEntity:data_set(args)
  if args ~= nil then
...
function ResultEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ResultEntity:match_set(args)
  if args ~= nil then
...
function ResultEntity:match_get()
...
function ResultEntity:stream(action, args, callopts)
...
---@param reqmatch ResultListMatch
---@param ctrl? table
---@return Result[]
---@return string? err
function ResultEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ResultEntity:_run_op(ctx, post_done)
  local utility = self._utility
