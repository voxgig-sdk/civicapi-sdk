...
The Lua SDK for the Civicapi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Election(nil):list(nil, nil)
...
local result, err = client:Civicapi(nil):load(
  { id = "test01" }, nil
)
