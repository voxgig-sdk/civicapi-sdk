...
class PollingDirectTest < Minitest::Test
  def test_direct_list_polling
    setup = polling_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-polling", setup[:live] ? "live" : "unit")
    if _should_skip
...
def polling_direct_setup(mockres)
