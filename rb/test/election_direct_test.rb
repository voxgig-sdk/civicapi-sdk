...
class ElectionDirectTest < Minitest::Test
  def test_direct_list_election
    setup = election_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-election", setup[:live] ? "live" : "unit")
    if _should_skip
...
def election_direct_setup(mockres)
