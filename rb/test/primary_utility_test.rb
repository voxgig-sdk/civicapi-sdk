...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CivicapiOmni.make_runner(TEST_JSON_FILE, CivicapiSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CivicapiOmni.errify(err)
    end
