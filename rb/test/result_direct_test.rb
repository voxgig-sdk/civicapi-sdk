...
class ResultDirectTest < Minitest::Test
  def test_direct_list_result
    setup = result_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-result", setup[:live] ? "live" : "unit")
    if _should_skip
...
def result_direct_setup(mockres)
