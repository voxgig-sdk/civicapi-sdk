...
class PollingEntity
  ...
  def make
    opts = @_entopts.dup
    PollingEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CivicapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Polling, Hash] the current Polling data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CivicapiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Polling fields)
  def match_get
  ...
  #
  # @param reqmatch [PollingListMatch, Hash, nil] match filter (any subset of
  #   Polling fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Polling>, Array] the matching Polling items; raises CivicapiError on failure
