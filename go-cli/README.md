...
boru-driven command-line client **and** interactive REPL for the Civicapi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/civicapi-cli)
make build
...
./civicapi-cli list polling

# 5. Override the API base URL for a single call
CIVICAPI_BASE=https://api.example.com ./civicapi-cli list election

# 6. No arguments -> interactive REPL
./civicapi-cli
civicapi> list election
civicapi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/civicapi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/civicapi-cli list election
   ```
...
Run with no arguments to open a REPL (prompt `civicapi>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `election`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/civicapi-cli`. |
...
(`./civicapi-cli <expr>`) and interactively (the REPL), and expressions compose the
