...
module CivicapiUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = CivicapiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CivicapiResponse.new(fetched)
    else
