...
class FtCtrl extends CivicapiControl
{
...
class FtClient
{
    ...
    private CivicapiUtility $utility;

    public function __construct(CivicapiUtility $utility, array $options)
    {
    ...
    public function get_utility(): CivicapiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CivicapiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CivicapiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CivicapiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CivicapiConfig::make_feature($name);
    ...
    private static function buildUrl(CivicapiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CivicapiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CivicapiResponse($fetched);
        }
    ...
    private function fail(CivicapiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CivicapiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
